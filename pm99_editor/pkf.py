...
class PKFFile:
    ...
    def search_strings(
        ...
        *,
        ...
        """Search entry payloads for one or multiple strings."""
        ...
        return searcher.search(patterns)
...
class PKFStringMatch:
    """Describe a string occurrence inside a PKF entry."""
...
class PKFStringSearcher:
    ...
    def search(
        ...
        """Return all occurrences of the provided string pattern(s)."""

        normalised = self._normalise_needles(patterns)
        matches: List[PKFStringMatch] = []

        for entry in self._pkf.list_entries():
            haystack = entry.raw_bytes
            haystack_cmp = haystack if self._case_sensitive else haystack.lower()
            entry_matches: List[PKFStringMatch] = []
                ...
                while True:
                    ...
                    entry_matches.append(
                        PKFStringMatch(
                            ...
                            match_offset=idx,
                            absolute_offset=entry.offset + idx,
                            needle=label,
                        )
                    )
                    search_start = idx + 1
            ...
            if entry_matches:
                entry_matches.sort(key=lambda m: m.match_offset)
                matches.extend(entry_matches)

        matches.sort(key=lambda m: m.absolute_offset)
        return matches

    def search_grouped(
        self, patterns: Union[str, bytes, Iterable[Union[str, bytes]]]
    ) -> Dict[str, List[PKFStringMatch]]:
        """Return matches grouped by needle label."""

        grouped: Dict[str, List[PKFStringMatch]] = {}
        for match in self.search(patterns):
