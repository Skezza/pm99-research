...
- Squad Number (byte 2): Direct value
- Position (byte 42): Double-XOR encoded (0=GK, 1=DEF, 2=MID, 3=FWD)
- Player Name (bytes 5-~40): Latin-1 text
- Attributes (bytes 50-61): Double-XOR encoded, clamped to 0-99

LIMITATIONS:
Due to variable-length record structure, the following fields require
additional reverse engineering and are not yet editable:
- Player Name (variable-length string affects all subsequent offsets)
- Date of Birth (metadata region, variable position)
- Height/Weight (metadata region, variable position)
- Nationality (metadata region, variable position)
        ...
        try:
            ...
            for match in matches:
                ...
                if len(text) >= 4 and text[0].isupper() and any(c.isalpha() for c in text):
                    # Prefer names with spaces or known patterns
...
class CoachRecord:
    ...
    def _extract_name(self) -> str:
        ...
        try:
            # From analyze_coaches.py: after outer XOR, inner strings with 32-bit XOR
            # Assume data is already outer-decoded
    ...
    def set_name(self, new_full_name: str):
        ...
        # For simplicity, update the full name string; actual impl needs to split and update fields
        # Placeholder: assume full name is concatenated
        self.full_name = new_full_name
        # TODO: Parse and update individual fields, pad accordingly
...
class PlayerRecord:
    ...
    def _extract_name(self) -> str:
        """Extract player name from bytes 5-40"""
        try:
            # Name region bytes 5-45
            name_region = self.raw_data[5:45]
            ...
            # Separators are typically 2 chars like }a, ta, ua, va, wa, xa, ya, za, etc.
            # Strategy: Split on separators and prefer the FULL NAME portion (usually ALL CAPS or proper format)
            
            # Split on common separators - look for lowercase+a before uppercase
            ...
            for part_idx, part in enumerate(parts):
                ...
                # Pattern: Given name(s) + Surname
                # Handle formats like: "David Robert BECKHAM", "FREDDY Eusebio RINCON", "José Santiago CAÑIZARES"
                patterns = [
                    # Given + Middle + ALL CAPS surname: "David Robert BECKHAM"
                    r'([A-ZÀ-ÿ][a-zà-ÿ]{2,15}\s+[A-ZÀ-ÿ][a-zà-ÿ]{2,15})\s+([A-ZÀ-ÿ]{3,20})',
                    # ALL CAPS given + Middle + Surname: "FREDDY Eusebio RINCON"
                    r'([A-ZÀ-ÿ]{3,15}\s+[A-ZÀ-ÿ][a-zà-ÿ]{2,15})\s+([A-ZÀ-ÿ]{3,20})',
                    # ALL CAPS multi-word: "JUAN CARLOS Rodríguez"
                    r'([A-ZÀ-ÿ]{3,15}\s+[A-ZÀ-ÿ]{3,15})\s+([A-ZÀ-ÿ][a-zà-ÿ]{3,20})',
                    # Simple given + ALL CAPS surname: "David BECKHAM"
                    r'([A-ZÀ-ÿ][a-zà-ÿ]{2,15})\s+([A-ZÀ-ÿ]{3,20})',
                    # Mixed case both: "David Beckham"
                ...
                for pattern_idx, pattern in enumerate(patterns):
                    ...
                    for match in matches:
                        ...
                        # Skip if given name is too short (likely a fragment)
                        if len(given) < 3:
                            continue
                        ...
                        for word in words:
                            if word.isupper() or (word[0].isupper() and all(c.isupper() or c.islower() or not c.isalpha() for c in word)):
                                # Valid word - check for garbage suffix
                                valid_part = ''
                                for i, c in enumerate(word):
                                    if i > 0 and c.islower():
                                        # Check if rest is garbage (all lowercase non-name characters)
                                        rest = word[i:]
                        ...
                        # Surname must be at least 3 chars
                        if clean_surname and len(clean_surname) >= 3:
                            full_name = f"{given} {clean_surname}".strip()
                            
                            # Validate: reasonable length, contains space, given name is reasonable
                            if 8 <= len(full_name) <= 40 and ' ' in full_name and len(given) >= 3:
                                # Scoring system (higher is better):
                                # - Prefer parts after separator (part_idx > 0): +200
                                # - Prefer patterns that match multi-word or proper format: +100
                                # - Add points for uppercase letters: +2 per uppercase
                                # - Add total length: +1 per character
                                # - Penalize very short given names: -50 if < 4 chars
                                score = 0
                                score += (part_idx * 200)  # Strongly prefer parts after separator
                                score += ((3 - pattern_idx) * 30)  # Prefer earlier patterns
                                score += (sum(1 for c in full_name if c.isupper()) * 2)
                                score += len(full_name)
                                if len(given) < 4:
                                    score -= 50  # Penalize very short given names
    ...
    def _extract_position(self) -> int:
        ...
        try:
            # Find 'aaaa' pattern (0x61 0x61 0x61 0x61) that marks name end
            name_end = None
            ...
            if not name_end:
                # Fallback: can't find marker
                return 0
            
            # Position is at offset +7 from the 'aaaa' marker
            pos_offset = name_end + 7
            if pos_offset < len(self.raw_data) - 20:  # Ensure we're before attributes
                pos_byte = self.raw_data[pos_offset]
                pos_value = pos_byte ^ 0x61  # Double-XOR decode
                
                # Validate: position should be 0-3
                if 0 <= pos_value <= 3:
                    return pos_value
                
                # Try offset +8 as backup
                pos_offset = name_end + 8
                if pos_offset < len(self.raw_data) - 20:
                    pos_byte = self.raw_data[pos_offset]
    ...
    def _extract_biography_name(self, data: bytes) -> str:
        ...
        try:
            ...
            patterns = [
                ...
                r'([A-Z]{2,15})\s+([A-Z][a-z]{2,20})',  # SURNAME Given
                r'([A-Z][a-z]{1,10})\s+([A-Z][a-z]{2,15})',  # Short names
    ...
    def to_bytes(self) -> bytes:
        ...
        # Update position (double-XOR) - dynamic offset
        # Find name-end marker to write position back
        name_end = None
...
def find_player_records(file_data: bytes) -> List[Tuple[int, PlayerRecord]]:
    ...
    # Format: [abbreviated][2char+a][FULL NAME][data bytes]
    ...
    while pos < len(file_data) - 1000:
        try:
            ...
            if 5000 < length < 200000:  # Large sections that may contain embedded records
                ...
                embedded_pattern = r'([A-Z][a-z]{3,15})([a-z~@\x7f]{1,2}a)([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?\s+[A-Z]{3,20})'
                ...
                for match in matches:
                    ...
                    if len(record_chunk) >= 62:  # Need at least this much for attributes
                        try:
                            ...
                            # Try parsing at different offsets to find alignment
                            for offset in range(min(50, len(candidate_chunk) - 80)):
                                ...
                                if len(test_chunk) >= 80:  # Need full record for validation
                                    try:
                                        ...
                                        # 2. Plausible team ID: 1-999 (0 is biography, >1000 unlikely)
                                        if not (1 <= test_record.team_id <= 999):
                                            continue
                                        
                                        # 3. All attributes 0-99 after decoding (critical for alignment)
                                        if not all(0 <= attr <= 99 for attr in test_record.attributes):
                                            continue
