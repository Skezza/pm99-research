#!/usr/bin/env python3
"""
Locate Dennis BERGKAMP's record by searching for full name in decoded data
"""
...
def main():
    ...
    target_name = b'Dennis BERGKAMP'  # Exact bytes for search
    
    print("🔍 Searching for exact 'Dennis BERGKAMP' in decoded sections...")
    
    pos = 0x400
    sections_found = 0
    bergkamp_found = False
        ...
        try:
            ...
            if 1000 < length < 100000:
                ...
                if target_name in decoded:
                    ...
                    with open('bergkamp_record.bin', 'wb') as f:
                        f.write(record_data)
                    print(f"\n💾 Full record saved to: bergkamp_record.bin")
                    
                    # Search for known values in this record
                    bergkamp_stats = [29, 84, 89, 79, 59, 96, 73, 99, 4, 90, 91, 62, 58, 87]  # Age, Rating, Attributes
                    print(f"\n🎯 Known Values Search in Record:")
                    for value in bergkamp_stats:
                        positions = []
    ...
    if not bergkamp_found:
        print("❌ Dennis BERGKAMP not found with exact match")
        ...
        while pos < len(file_data) - 1000:
            try:
                ...
                if 1000 < length < 100000:
                    ...
                    # Look for "BERGAMP" anywhere
                    if b'BERGAMP' in decoded.upper():
                        start_offset = decoded.upper().find(b'BERGAMP')
                        ...
                        with open('bergkamp_partial.bin', 'wb') as f:
                            f.write(extract_data)
                        print(f"Extract saved to: bergkamp_partial.bin")
                        
                        # Show context
                        context = extract_data.decode('latin-1', errors='ignore')
                        print(f"Context around BERGAMP:\n{context}")
                        break
