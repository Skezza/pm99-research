#!/usr/bin/env python3
"""
Locate the exact Peter THORNE record by searching for full name in decoded data
"""
...
def main():
    ...
    target_name = b'Peter THORNE'  # Exact bytes for search
    
    print("🔍 Searching for exact 'Peter THORNE' in decoded sections...")
        ...
        try:
            ...
            if 1000 < length < 100000:
                ...
                if target_name in decoded:
                    ...
                    with open('thorne_record.bin', 'wb') as f:
                        f.write(record_data)
                    print(f"\n💾 Full record saved to: thorne_record.bin")
                    
                    # Search for known values in this record
                    thorne_stats = [25, 70, 66, 63, 74, 64, 73, 84, 28, 69, 65, 73, 69, 80]  # Age, Rating, Attributes
                    print(f"\n🎯 Known Values Search in Record:")
                    for value in thorne_stats:
                        positions = []
    ...
    if not thorne_found:
        print("❌ Peter THORNE not found with exact match")
        ...
        while pos < len(file_data) - 1000:
            try:
                ...
                if 1000 < length < 100000:
                    ...
                    # Look for "THORNE" anywhere
                    if b'THORNE' in decoded.upper():
                        start_offset = decoded.upper().find(b'THORNE')
                        ...
                        with open('thorne_partial.bin', 'wb') as f:
                            f.write(extract_data)
                        print(f"Extract saved to: thorne_partial.bin")
                        
                        # Show context
                        context = extract_data.decode('latin-1', errors='ignore')
                        print(f"Context around THORNE:\n{context}")
                        break
